...
- `risk_table.json`: time points and full cell records, including raw text and confidence when available.
...
- `pixel_region` is currently `null` in exported cells. Stable cell IDs support editing now; per-cell source boxes can be added by a later localization stage.
